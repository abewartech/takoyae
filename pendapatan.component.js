...
export const PendapatanScreen = ({navigation}) => {
    ...
    db.transaction(tx => {
      tx.executeSql(
        `SELECT * FROM penjualan WHERE tgl BETWEEN '${moment(
          global.config.date,
        ).format('YYYY-MM-DD')}' AND '${moment(global.config.dateTo).format(
          'YYYY-MM-DD',
        )}'`,
        [],
        (tx, results) => {
    ...
    await RNPrint.print({
      ...
      <body>
        <div style="width: 100%; text-align: center;">
          ...
          <img src="https://i.imgur.com/ivGxYw3.png" width="90" height="100" align=left>
            Takoyae Ramen
          </h1>
        ...
        <h2 style="font-weight: bold;">
          ...
          >
          ${moment(global.config.date).format('DD MMMM YYYY')} - ${moment(
        global.config.dateTo,
      ).format('DD MMMM YYYY')}</sup
          >
        </h2>
